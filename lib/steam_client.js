...
SteamClient.prototype.setProxyHttp = function (proxyHttp) {
    this._config = {
...
SteamClient.prototype.setProxySocks5 = function (host, port, username, password) {
    this._config = {
        'proxy' : 'socks5',
        'host' : host,
        'port' : port,
...
SteamClient.prototype.connect = function (config) {
    this.disconnect();
    ...
    if (this._config.proxy=='socks5') {
        this.emit('debug', 'use proxy ' + this._config.host + ':' + this._config.port);
        this._connection = new (require('./connection_sock5'))({
            socksHost: this._config.host,
            socksPort: this._config.port,
            socksUsername: this._config.socksUsername,
            socksPassword: this._config.socksPassword
        });
    }else if (false && this._config.proxy_http) {
        this.emit('debug', 'use proxy_http ' + this._config.proxy_http);
        this._connection = new (require('./connection_http'))(this._config);
    }else{
    ...
    this._connection.on('debug', function (mess) {
        self.emit('debug', mess);
    });

    this._connection.setTimeout(this._config.connectTimeOut, function () {
